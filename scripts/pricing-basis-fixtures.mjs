...
check(batchEntry.includes("batch-workspace-profile-card")
  && batchEntry.includes("<BatchPricingCard")
  && pricingCard.includes("<SummaryRow title=\"Pricing\"")
  && pricingCard.includes("verticalTitleWhenExpanded titleStyle={pricingSectionLabel}")
  && pricingCard.includes("<BatchPricingBasisWorkspace compact")
  && !pricingCard.includes("batch-pricing-header-card")
  && !pricingCard.includes("<SummaryRow title=\"Basis\"")
  ...
  "U4-FE-10 Batch reference and Pricing Basis controls live in an independent Pricing SummaryRow card")
check(appCss.includes(".batch-profile-pricing-card {\n  width: auto;\n  min-width: 260px;\n  max-width: 360px;\n  flex: 1 1 300px;\n}")
  && selector.indexOf("batch-pb-batch-row") > 0
  && selector.indexOf("batch-pb-batch-row") < selector.indexOf("batch-pb-date-row")
  && selector.indexOf("batch-pb-date-row") < selector.indexOf("batch-pb-release-row")
  && selector.indexOf("batch-pb-governed-actions") < selector.indexOf("batch-pb-release-row")
  && !appCss.includes("width: 340px;")
  ...
  && pricingCard.includes("<SummaryRow title=\"Pricing\"")
  && pricingCard.includes("verticalTitleWhenExpanded")
  && pricingCard.indexOf("<BatchPricingBasisWorkspace compact") > pricingCard.indexOf("<SummaryRow")
  ...
  "U4-FE-11 desktop Pricing swaps its horizontal summary for a vertical label around a two-row compact workspace sized like its sibling cards")
...
check(profileBar.includes("<SummaryRow title=\"Customer\"")
  && profileBar.includes("<SummaryRow title=\"Commercials\"")
  && profileBar.includes("<SummaryRow title=\"Terms\"")
  && profileBar.indexOf("<SummaryRow title=\"Terms\"") > profileBar.indexOf("<SummaryRow title=\"Commercials\"")
  && !profileBar.includes("gridColumn:\"1 / -1\"")
  && profileBar.includes("gridTemplateColumns:\"auto 56px\",gridTemplateRows:\"21px 21px 21px\"")
  && !profileBar.includes("termHdr")
  && profileBar.includes("gridTemplateRows:\"12px 23px 23px\"")
  && profileBar.includes("rowGap:0")
  && profileBar.includes("_termOverrideCount")
  && profileBar.includes("_commercialOverrideCount")
  && profileBar.includes("statusTone={_commercialOverrideCount?\"warning\":\"neutral\"}")
