...
check(appCss.includes(".batch-profile-pricing-card {\n  width: auto;")
  && appCss.includes("min-width: 220px;")
  && appCss.includes("max-width: 240px;")
  && appCss.includes("flex: 0 1 240px;")
  ...
  "U4-FE-11 desktop Pricing swaps its horizontal summary for a vertical label around the compact workspace")
check(profileBar.includes("<SummaryRow title=\"Actions\"")
  && profileBar.includes("facts={[\"Import profile\",\"New batch\"]}")
  && profileBar.includes("onClick={copyCostingToProfile}")
  && profileBar.includes("onClick={startNewBatch}")
  && !profileBar.includes("onClick={importConstrFromSpec}")
  && !profileBar.includes("+ Constr"),
  "U4-FE-11a Batch profile actions share the disclosure format without duplicating Construction Library")
...
check(profileBar.includes("Commercials (including Terms)")
  && profileBar.includes("<SummaryRow title=\"Customer\"")
  ...
  "U4-FE-13 Customer and Commercials collapse while Terms stays integrated with all guarded controls")
