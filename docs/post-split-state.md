...
| ~~D-3~~ | Backup silently discards the two raw-string keys | High | **FIXED — verified 2026-09-01.** Both legs: `useQuoteActions.js:47-50` falls back to the raw string when `JSON.parse` throws; `:124-133` passes strings through instead of re-stringifying, so base64 is not quote-corrupted. Absent vs explicit-`null` is distinguished, and a pre-fix file raises a `window.alert`. **Browser round trip:** backup written with `cbb_template` as a 103,488-char base64 string (not null, zero null-valued keys); restored byte-identical (same length, same hash) |
| ~~D-4~~ | Identity freeze is lost on reload while the batch survives | High | **DISSOLVED — not fixed, removed.** C6 (`6bd2903`) deleted `specCommitted` and the identity-freeze rendering. `specCommitted` appears nowhere in `src/` except historical comments, so there is no freeze to lose on reload. The read-only rule now lives in the Batch Context bar's own mode test. **Nothing here was repaired; the mechanism the defect described stopped existing** |
...
| ~~D-19~~ | `exportExcelFull` throws `ReferenceError` on every call | High | **FIXED — verified 2026-09-01.** `qty` was a column pair for a field the data model never had; the row map was cut to match the 10-column header (`excel.js:107-114`). `locations` became `_locations`, read from `cbb_locations` with a fallback to the `LOCATIONS` constant (`:136-146`). **Browser check:** with no stored template and the backend leg forced to fail, the client-side fallback ran to completion - zero `ReferenceError`, and it produced `CFB_Quote_..._20260901.xlsx` as a blob. ⚠️ The module header at `excel.js:11-18` still describes this bug as live; correcting that comment is a separate commit |
...
| ~~D-24~~ | G1 identity guards gate on row count, not on whether the profile holds an identity | High | **FIXED at C5** (`7e9bade`). `useCostingBatchBridge.js:484` now asks the profile, not the row count: `_profileHasIdentity=!!(batchProfile.client\|\|batchProfile.sector)`. An empty grid with a populated profile is treated as committed, so all four mismatch guards fire. The guards read `specRaw` (C5), so the resolved spec cannot compare the profile with itself. All four were kept unchanged by ruling as D-24 defence-in-depth |
