...
| B | Done | Applied live as `20260918040738_seed_nagpur_limited_beta_masters`: 19 approved Sectors, 17 Rate entries including exact `25`, one approved Nagpur Freight lane bound to `G0080-001-03`, one automatic `FMCG-FOOD` Release and four published/adopted starter Constructions. `18J` and job work remain excluded. Backend migration commit `a4f6c52`. |
| C | Deployed — persona smoke waiting | `calculate-batch-row` version 1 is ACTIVE with JWT verification enabled; executor fixtures pass and deployment logs were clean. Maker and Checker invitations remain unconfirmed with no application identities, so no capability was granted and the different-person Maker → Checker smoke cannot yet run. Recheck 2026-09-18 (read-only): still unaccepted; additionally `app_private.attestation_keys` is empty, so live Calculate cannot verify until the Product Owner provisions the matching database key. See the operating sheet recheck. |
