...
| Customer Families / Parties — **propose, approve, edit** | ~~No governed path~~ **correction, found while writing the mutations packet: already governed.** `pg_policies` shows `customer_families_insert`/`parties_insert` already permit a direct `INSERT` with `status='proposed'` for any caller holding `make_quote`, and `UPDATE` already permits `manage_customer_master` holders to approve/edit directly | A thin Flask POST/PATCH route forwarding the caller's token to a plain table insert/update | **None at the database layer.** No wrapper, no new SQL. This report's earlier draft conflated create/approve with the three real gaps below before checking `pg_policies`. Not built in this pass — only the read screen was. |
| Customer Families — **merge, reassign, graduate** | `app_private.merge_families`, `app_private.reassign_party_family`, `app_private.graduate_party` — all exist and are pgTAP-tested, but **none has a `public` invoker wrapper** | Public wrapper RPCs following the established P2-6 pattern (SECURITY DEFINER in `app_private`, thin `public` shim), plus one Flask route per action forwarding the caller's token | **Real gap.** These three functions are architecturally unreachable from any caller today — not just unrouted in Flask, but not exposed to PostgREST at all, by the same `app_private`-is-not-in-the-Data-API's-exposed-schema-list rule that already protects `is_admin()` and the identity RPCs. Full design in `u1-customer-family-mutations-packet.md` — not implemented in this pass. |
